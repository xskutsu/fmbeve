...
import type { FMBEData } from "../../fmbe/types";
import type { MolangVariableMap } from "../../molang/types";

export function updateFMBEMatrix(
	mesh: Mesh,
	data: FMBEData,
	variables: MolangVariableMap | null
): void {
	const px: number = parseFMBEValue(data.position.x, 0, variables);
	const py: number = parseFMBEValue(data.position.y, 0, variables);
	const pz: number = parseFMBEValue(data.position.z, 0, variables);
	const rx: number = parseFMBEValue(data.rotation.x, 0, variables) * DEG_TO_RAD;
	const ry: number = -parseFMBEValue(data.rotation.y, 0, variables) * DEG_TO_RAD;
	const rz: number = -parseFMBEValue(data.rotation.z, 0, variables) * DEG_TO_RAD;
	const bx: number = parseFMBEValue(data.basePosition.x, 0, variables);
	const by: number = parseFMBEValue(data.basePosition.y, 0, variables);
	const bz: number = parseFMBEValue(data.basePosition.z, 0, variables);
	const s: number = parseFMBEValue(data.scale, 1, variables);
	const es: number = parseFMBEValue(data.extend.scale, 1, variables);
	const ex: number = parseFMBEValue(data.extend.rotation.x, -90, variables) * DEG_TO_RAD;
	const ey: number = -parseFMBEValue(data.extend.rotation.y, 0, variables) * DEG_TO_RAD;
	...
	const scaleY: number = mesh.geometry.type === "BoxGeometry" ? es : 1;
	...
	mesh.matrix.set(f00, f01, f02, tx, f10, f11, f12, ty, f20, f21, f22, tz, 0, 0, 0, 1);
